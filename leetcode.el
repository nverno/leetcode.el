...
(defmacro leetcode--unless-cached (problem-id buf-type &rest body)
  "Return cached BUF-TYPE for PROBLEM-ID if non-nil or do BODY."
  (declare (indent 2))
...
(defun leetcode-result-buffer (problem-id &optional create)
  "Return result buffer for PROBLEM-ID and SOURCE-BUFFER.
If CREATE is non-nil, create new buffer when necessary."
  (leetcode--unless-cached problem-id 'result
    (let ((buf-name (leetcode--make-buffer-name problem-id "result")))
      (or (get-buffer buf-name)
          (when create
            (with-current-buffer (get-buffer-create buf-name)
              (leetcode-result-mode)
...
If CREATE is non-nil, create new buffer when necessary."
  (leetcode--unless-cached problem-id 'test
    (let ((buf-name (leetcode--make-buffer-name problem-id "test")))
      (or (get-buffer buf-name)
          (and create (get-buffer-create buf-name))))))

(defun leetcode-detail-buffer (problem-id &optional create)
  "Return detail buffer for PROBLEM-ID.
If CREATE is non-nil, create new buffer when necessary."
  (leetcode--unless-cached problem-id 'detail
    (let ((buf-name (leetcode--make-buffer-name problem-id "detail")))
      (or (get-buffer buf-name)
          (and create (get-buffer-create buf-name))))))
...
(defun leetcode--buffer-type (&optional buffer)
  ...
  (with-current-buffer (or buffer (current-buffer))
    (cond (leetcode-solution-minor-mode 'code)
          ((eq 'leetcode-detail-mode major-mode) 'detail)
          ((eq 'leetcode-result-mode major-mode) 'result)
...
(defun leetcode-view-problem (problem-id)
  ...
  (save-window-excursion (leetcode-show-problem problem-id)))
...
(defvar-keymap leetcode-base-map
...
(defvar-keymap leetcode-detail-mode-map
  ...
  :parent leetcode-base-map
  "q" #'quit-window)

(define-derived-mode leetcode-detail-mode special-mode "LCd"
...
(defvar leetcode-compilation-error-regexp-alist-alist
  '((leetcode
     "Line \\([0-9]+\\): Char \\([0-9]+\\): \\([^:]+\\):"
     (leetcode--result-source) 1 2 (2) 0 (3 compilation-error-face))
    (racket
...
(defvar-keymap leetcode-result-mode-map
  :doc "Keymap in `leetcode-result-mode.'"
  :parent leetcode-base-map)

(define-derived-mode leetcode-result-mode text-mode "LCr"
